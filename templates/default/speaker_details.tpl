...
{include file="header.tpl" title="Speakers Details"}
		...
		<span class="left-img"><img src="{#ROOT#}/images/speakers/{$speaker.picture}" alt="{$speaker.name}" /></span>
		...
		<p><strong>Presentation Time</strong>: {$talks[outer].dayofweek}, {$talks[outer].date}, at <a href="{#ROOT#}/schedule.php#{$talks[outer].hour}">{$talks[outer].start_time} - {$talks[outer].end_time}</a></p>
