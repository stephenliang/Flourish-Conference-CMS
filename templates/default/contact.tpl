{config_load file=default.conf section=$site}
{include file="header.tpl" title="Contact Us"}
			<h1 class="content-title">Contact Information</h1>

			<div class="essay">
				<span class="long-img"><a href="{#ROOT#}/images/penguin-escalator.jpg"><img src="{#ROOT#}/images/penguin-escalator.jpg" class="long" /></a><div class="caption">Flourish Promotional Event at Student Center East at UIC.</div></span>
				<h3>General Conference Information</h3>
				<a href="mailto:info@flourishconf.com">info@flourishconf.com</a>
				
				<h3>Flourish Event Planners</h3>
				<a href="mailto:event@flourishconf.com">event@flourishconf.com</a>

				<h3>Conference Business Contact</h3>
				<a href="mailto:stephen@flourishconf.com">stephen@flourishconf.com</a>

				<h3>Flourish listserv</h3>
				e-mail with a body containing <strong>subscribe FLOURISH</strong> to <a href="mailto:flourish@listserv.uic.edu">flourish@listserv.uic.edu</a>

				<h3>Other Social Media</h3>
				<a href="http://twitter.com/flourishconf">Follow @flourishconf on twitter</a><br />
				Use hashtag <a href="http://twitter.com/#search?q=%23flrsh12">#flrsh12</a> on twitter<br />
				<a href="http://facebook.com/flourishconf">Like us on facebook</a><br />

				<div style="clear: both;"></div>
			</div>
{include file="footer.tpl"}
